...
interface E2EStoreState {
  words: E2EWord[]
  daily: { learned: number; quizzed: number; reviewed: number; date?: string }
  todayPack: { date?: string; recommendedIds: string[] }
  streakData: { current: number; longest: number; lastStudyDate?: string }
...
async function readStore(page: Page): Promise<E2EStoreState> {
...
// ── 1. 定级 → 今日包个性化 ────────────────────────────────────────────────
// 2026-06-25：TodayBento 恢复挂载时 buildTodayPack（与 HomeScreen 同源），故此例重新启用。
test('定级 B1(band4) → 推荐词落在 CEFR 邻近窗口且今日页出现推荐区', async ({ page, request }) => {
  ...
  await seed(page, { profile: { onboarded: true, skipped: false, targetExam: null, path: 'full', band: 4, dailyGoal: 12, userLevel: 'intermediate' } })
  await page.goto('/today')
  // 按 path 编排后：全面 path 首卡「今日新词」即推荐区入口
  await expect(page.getByText('今日新词')).toBeVisible()
  // 挂载 effect 异步重建今日包（buildTodayPack）→ 等其落库再读，避免读到空包
...
test('学新词「认识」→ 状态 learning + 今日进度 +1', async ({ page }) => {
  ...
  const st = await readStore(page)
  expect(st.words.find((w) => w.id === 'alpha')?.state).toBe('learning')
...
test.fixme('测验答错 → 状态 weak + 错题本出现该题', async ({ page, request }) => {
  ...
  // PracticeSession 首题为 en→中（选项=中文释义），正解 = 中文释义
  const correctZh: string = (dict.definitions[0].definitionZh ?? '').trim()
  ...
  await expect(page.locator('.pr-v2 .prompt .word', { hasText: 'inevitable' })).toBeVisible()

  const opts = page.locator('.pr-v2 .opt')
  await expect(opts).toHaveCount(4)
  const n = await opts.count()
  for (let i = 0; i < n; i++) {
    const t = (await opts.nth(i).innerText()).trim()
    if (!correctZh || !t.includes(correctZh)) { await opts.nth(i).click(); break }
  }
  await expect(page.getByText('答错了')).toBeVisible()

  const st = await readStore(page)
  expect(st.words.find((w) => w.id === 'inevitable')?.state).toBe('weak')
...
// ── 7. 次日重置 ──────────────────────────────────────────────────────────
// 2026-06-25：TodayBento 恢复挂载时 buildTodayPack，故重新启用；CTA 文案锚点更新为现 UI「继续学习」。
test('次日进入 → 进度归零、今日包重新生成且不同', async ({ page }) => {
  ...
  await page.goto('/today')
  // 次日重置：今日动线 CTA「继续学习 →」(TodayBento)；进度归零见下方 todayPack 重建断言
  await expect(page.getByRole('button', { name: /继续学习/ })).toBeVisible()
  // buildTodayPack 在挂载后异步重建（seed 注入昨日包 → 守卫失效 → 重建为今天）
  await page.waitForFunction(
    ({ key, d }) => JSON.parse(window.localStorage.getItem(key) ?? '{"state":{}}').state.todayPack?.date === d,
    { key: KEY, d: today() },
  )
...
test('词详情「加入学习」→ 状态机入库 + 词库可见', async ({ page }) => {
  ...
  await expect
    .poll(async () => {
      const st = await readStore(page)
      return st.words.find((w) => w.id === 'inevitable')?.state
    })
    .toBe('learning')
  const st = await readStore(page)
  expect(st.words.find((w) => w.id === 'inevitable')?.source).toBe('lookup')
  expect(st.daily.learned).toBe(1)
