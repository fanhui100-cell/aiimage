...
4. **Browser sweep (R13) — NOW RUN (2026-06-25).** `npx playwright test` was executed in a dev-server env: **7 passed / 5 `test.fixme` (documented) / 0 failed**. All 8 original failures were proven pre-existing **stale specs** (not R11/R12 regressions — verified by reverting the touched components to the pre-R11 commit and re-running to the same failures). **3 are now fixed** (`:136` stale wait-anchor; `:72`+`:198` after restoring TodayBento's idempotent `buildTodayPack` mount effect — a real behavior gap from the 界面优化4 migration). **5 remain `test.fixme`** with in-code reasons: a question-bank fixture (`:106`), v3-iframe internal rebuilds (`:45`/`:131`/`:195`), and one product decision (`:161` orphaned lexigraph red-edge feature). Full per-test analysis in `reports/e2e-stale-audit-2026-06-25.md`. Static route + DB-safety invariants remain PASS (`smoke:full-routes`). These e2e specs cover the v1 lexiStore learning loop (orthogonal to the v2 paper layer); R11/R12 is covered by tsc/eslint/build + `smoke:*`.
...
**Still not "complete" — but only because of TOEFL content/spec blocks, not verification debt.** TOEFL `reading`/`listening`/`speaking`/`build_a_sentence` remain blocked (each explicit, documented, with a concrete external unblocking input — ETS spec / original listening content + audio / speaking renderer+recording+privacy / equivalence-scoring). **6 of 7 exams (zhongkao, gaokao, cet4, cet6, kaoyan, SAT) are fully active and release-ready**; R11 (server-authoritative mock papers — assembly + scoring) and R12 (learning-face v2 overlay) are DONE. Non-blocking external items: CET-6 ~1257-word licensed-list gap, AI/Vision provider stubs (mock fallback, honest `isEstimate`), worktree cleanup, and the 5 remaining fixme'd v1-loop e2e specs (3 of 8 now fixed; 1 product decision — orphaned lexigraph red-edge — in `e2e-stale-audit-2026-06-25.md`).
