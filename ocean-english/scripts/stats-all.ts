...
const LVN: Record<number, string> = { 1: '中考', 2: '高考', 3: 'CET4', 4: 'CET6', 5: '考研', 6: '托福', 7: 'SAT' }
...
async function main() {
  ...
  const types = new Map<string, number[]>()  // type -> [lv1..lv7] counts
  const lvTot = [0, 0, 0, 0, 0, 0, 0]
  for (const r of qb) {
    const lvTag = (r.theme_tags ?? []).map(x => /^lv([1-7])$/.exec(x)?.[1]).find(Boolean)
    const lv = lvTag ? Number(lvTag) : 0
    if (!types.has(r.type)) types.set(r.type, [0, 0, 0, 0, 0, 0, 0])
    if (lv >= 1 && lv <= 7) { types.get(r.type)![lv - 1]++; lvTot[lv - 1]++ }
  }

  // ── 输出 ──
  console.log('\n╔══════════════ 词渊 LexiVerse 全量数据统计 ══════════════╗\n')
  console.log('【词库】 单词 ' + W + ' 个')
  console.log('  按主等级: ' + [1, 2, 3, 4, 5, 6, 7].map(l => `${LVN[l]} ${byLv.get(l) ?? 0}`).join(' · ') + (noLv ? ` · 未分级 ${noLv}` : ''))
  ...
  const head = '题型'.padEnd(22) + [1, 2, 3, 4, 5, 6, 7].map(l => padL(LVN[l], 6)).join('') + padL('合计', 8)
  console.log('  ' + head)
