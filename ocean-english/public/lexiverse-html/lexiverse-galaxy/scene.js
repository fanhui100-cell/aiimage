...
export function createGalaxy(container, graph, opts = {}) {
  ...
  const SOLID_OP    = { mastered: 1.0,  unlockable: 0.92, locked: 0.78 };
  const SOLID_COLOR = { mastered: 1.0,  unlockable: 0.72, locked: 0.32 };
  // glow halo opacity — locked has only an ember; lit shines
  const GLOW_F      = { mastered: 0.95, unlockable: 0.36, locked: 0.06 };
  // glow halo size — lit grows a touch larger than the body, locked tight
  const GLOW_SCALE  = { mastered: 1.38, unlockable: 1.04, locked: 0.62 };
  ...
  function frame(now) {
    ...
    for (const o of nodeObjects.values()) {
      ...
      // Distance-based glow scale compensation: clamp minimum world-space glow size
      // so distant planets stay visible instead of vanishing to sub-pixel points.
      const camDist = camera.position.distanceTo(o.grp.position);
      const minGlowWorld = camDist * 0.032;
      for (const gg of o.glows) {
        gg.mat.opacity = gOp;
        const nominalScale = gg.base * GLOW_SCALE[n.status];
        gg.sp.scale.setScalar(Math.max(nominalScale, minGlowWorld));
      }
