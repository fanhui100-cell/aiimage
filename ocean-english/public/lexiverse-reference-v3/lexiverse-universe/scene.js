...
export function createUniverse(container, opts = {}) {
  ...
  scene.fog = new THREE.FogExp2('#04040a', 0.000028);
  ...
  controls.minDistance = 24; controls.maxDistance = 15000;
  ...
  // ── far stardome: always behind everything, fills view at any zoom-out ──
  addStarDome(scene, makePointsMaterial);
  ...
  for (const g of C.GALAXIES) {
    ...
    const haloMat = new THREE.SpriteMaterial({ map: haloTex, color, transparent: true, opacity: 0.58, blending: THREE.AdditiveBlending, depthWrite: false });
  ...
  // ── 金色学习轨道: 串联等级带 7 座星系 (初中→SAT) ──
  const sectorLines = []; // { ids: [galaxyId...], obj } — 随所属星系一起显隐
  {
    const belt = C.GALAXIES.filter(g => g.pathOrder).sort((a, b) => a.pathOrder - b.pathOrder);
    if (belt.length > 1) {
      const pts = belt.map(g => { const p = positions.get(g.id); return new THREE.Vector3(p.x, p.y, p.z); });
      const curve = new THREE.CatmullRomCurve3(pts, false, 'catmullrom', 0.35);
      const line = new THREE.Line(
        new THREE.BufferGeometry().setFromPoints(curve.getPoints(180)),
        new THREE.LineBasicMaterial({ color: 0xffc85a, transparent: true, opacity: 0.30, blending: THREE.AdditiveBlending, depthWrite: false })
      );
      line.renderOrder = -5;
      scene.add(line);
      sectorLines.push({ ids: belt.map(g => g.id), obj: line });
      // 轨道上的微光点
      const dotCount = 90;
      const dpos = new Float32Array(dotCount * 3), dcol = new Float32Array(dotCount * 3), dsiz = new Float32Array(dotCount), dpha = new Float32Array(dotCount);
      for (let i = 0; i < dotCount; i++) {
        const p = curve.getPoint(i / (dotCount - 1));
        dpos[i * 3] = p.x; dpos[i * 3 + 1] = p.y; dpos[i * 3 + 2] = p.z;
        dcol[i * 3] = 1; dcol[i * 3 + 1] = 0.78; dcol[i * 3 + 2] = 0.35;
        dsiz[i] = 2.4 + (i % 5 === 0 ? 2.6 : 0); dpha[i] = (i / dotCount) * TAU;
      }
      ...
      dgeo.setAttribute('aPhase', new THREE.BufferAttribute(dpha, 1));
      const beltDots = new THREE.Points(dgeo, makePointsMaterial({ opacity: 0.85, drift: 4, sizeScale: 1 }));
      scene.add(beltDots);
      sectorLines.push({ ids: belt.map(g => g.id), obj: beltDots });
    }
    // 其它星座区也用自己的颜色连线 (类别内部串联)
    for (const c of C.CONSTELLATIONS) {
      if (c.id === 'level-belt') continue;
      const gs = C.GALAXIES.filter(g => g.constellationId === c.id);
      if (gs.length < 2) continue;
      const cpts = gs.map(g => { const p = positions.get(g.id); return new THREE.Vector3(p.x, p.y, p.z); });
      const ccurve = new THREE.CatmullRomCurve3(cpts, false, 'catmullrom', 0.4);
      const cline = new THREE.Line(
        new THREE.BufferGeometry().setFromPoints(ccurve.getPoints(64)),
        new THREE.LineBasicMaterial({ color: new THREE.Color(c.color), transparent: true, opacity: 0.20, blending: THREE.AdditiveBlending, depthWrite: false })
      );
      cline.renderOrder = -5;
      scene.add(cline);
      sectorLines.push({ ids: gs.map(g => g.id), obj: cline });
    }
  }

  // ── camera tween ─────────────────────────────────────────────────────────
  let currentGalaxyId = null;
  let examFactors = null; // Map<galaxyId, 0..1> when an exam route is active
  ...
  function ensureLabel(id) {
    ...
    if (!el) {
      ...
      el.className = 'lv-label' + (o.galaxy.pathOrder ? ' lv-mini' : '');
      const num = o.galaxy.pathOrder ? `<i>${String(o.galaxy.pathOrder).padStart(2, '0')}</i> ` : '';
      const wc = o.galaxy.wordCount ? ` · ${o.galaxy.wordCount.toLocaleString()}` : '';
      el.innerHTML = `${num}<b style="color:${o.galaxy.colorTheme}">${o.galaxy.title}</b><br><span>${o.galaxy.titleZh}${wc}</span>`;
  ...
  function frame() {
    ...
    for (const o of nodeObjects.values()) {
      ...
      const exam = examFactors ? (examFactors.get(o.galaxy.id) ?? 1) : 1;
      const mute = (isMuted ? 0.32 : (isHovered ? 1.25 : 1)) * exam;
      o.diskMat.uniforms.uMute.value += (mute - o.diskMat.uniforms.uMute.value) * 0.15;

      const haloBase = isSelected ? 0.72 : isHovered ? 0.68 : 0.56;
      o.haloMat.opacity = (haloBase + (s - 0.5) * 0.18) * (isMuted ? 0.4 : 1) * (0.3 + 0.7 * exam);
    ...
    // labels: hovered + selected + 等级带常显编号
    const wanted = new Set();
    if (hoveredId) wanted.add(hoveredId);
    if (currentGalaxyId) wanted.add(currentGalaxyId);
    for (const o of nodeObjects.values()) if (o.grp.visible) wanted.add(o.galaxy.id); // 所有星系常显名字
    ...
    for (const id of wanted) {
      ...
      el.style.opacity = (id === hoveredId || id === currentGalaxyId) ? '1' : '0.78';
    }
  ...
  return {
    ...
    setFilteredGalaxies(visibleSet) {
      ...
      // 连线与所属星系同生同灭: 只要该组任一星系可见则线可见
      for (const s of sectorLines) {
        s.obj.visible = !visibleSet || s.ids.some((id) => visibleSet.has(id));
      }
    },
    // exam route highlight: pass Map<galaxyId,0..1> (coverage factor) or null to clear
    setExamMode(factorMap) { examFactors = factorMap || null; },
    dispose() {
...
function distributeGalaxies(galaxies) {
  ...
  const placed = [];
  const out = new Map();
  const R_XZ = 2700, R_Y = 1000, MIN_DIST = 430;
  for (const g of galaxies) {
    if (g.fixedPos) { const fp = { x: g.fixedPos.x * 1.4, y: g.fixedPos.y * 1.4, z: g.fixedPos.z * 1.4 }; placed.push(fp); out.set(g.id, fp); continue; }
    let p = null;
...
function spiralParamsFor(g, rnd) {
  // 星系半径随词量大幅拉开对比: 小星系~r38, 托福13,477词~r225 (近 6×)
  const baseR = g.wordCount ? 30 + (g.wordCount / 10500) * 196 : 40 + rnd() * 16;
  const p = {
    count: Math.min(3200, Math.round(1250 * (baseR / 55))), bulge: Math.round(230 * (baseR / 55)),
    radius: baseR, arms: 2, spin: 5.4,
...
/* ── ~17,000 mid background particles (distant planets/stars), GPU drift ── */
function addBackgroundField(scene, makeMat) {
  ...
  const tmp = new THREE.Color();
  for (let i = 0; i < n; i++) {
    const r = 700 + Math.pow(Math.random(), 0.7) * 9500;
    ...
    if (!planet) tmp.multiplyScalar(0.72 + Math.random() * 0.42);
    col[i * 3] = tmp.r; col[i * 3 + 1] = tmp.g; col[i * 3 + 2] = tmp.b;
    siz[i] = planet ? 10 + Math.random() * 15 : 1.9 + Math.random() * 4.2;
  ...
  geo.setAttribute('aSize', new THREE.BufferAttribute(siz, 1));
  geo.setAttribute('aPhase', new THREE.BufferAttribute(pha, 1));
  const mat = makeMat({ opacity: 1.0, drift: 7.5, sizeScale: 1 });
  scene.add(new THREE.Points(geo, mat));
  return mat;
}

/* ── far stardome: a huge shell of stars rendered behind everything so the
   background is full of stars at ANY zoom level (depthTest off, no fog) ──── */
function addStarDome(scene, makeMat) {
  const n = 9000;
  const pos = new Float32Array(n * 3), col = new Float32Array(n * 3), siz = new Float32Array(n), pha = new Float32Array(n);
  const tints = ['#dfe9ff', '#ffe7cf', '#ffd9e8', '#e2fcff', '#ffffff', '#cdd8ff'].map(h => new THREE.Color(h));
  const tmp = new THREE.Color();
  for (let i = 0; i < n; i++) {
    const r = 12000 + Math.random() * 7000;          // 12k–19k, always beyond camera
    const th = Math.random() * TAU, ph = Math.acos(2 * Math.random() - 1);
    pos[i * 3]     = Math.sin(ph) * Math.cos(th) * r;
    pos[i * 3 + 1] = Math.cos(ph) * r;
    pos[i * 3 + 2] = Math.sin(ph) * Math.sin(th) * r;
    const bright = Math.random() < 0.12;
    tmp.copy(tints[(Math.random() * tints.length) | 0]).multiplyScalar(bright ? 1 : 0.6 + Math.random() * 0.35);
    col[i * 3] = tmp.r; col[i * 3 + 1] = tmp.g; col[i * 3 + 2] = tmp.b;
    siz[i] = (bright ? 70 : 34) + Math.random() * 40;  // large because very far (perspective shrinks)
    pha[i] = Math.random() * TAU;
  }
  const geo = new THREE.BufferGeometry();
  geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
  ...
  geo.setAttribute('aSize', new THREE.BufferAttribute(siz, 1));
  geo.setAttribute('aPhase', new THREE.BufferAttribute(pha, 1));
  const mat = makeMat({ opacity: 0.95, drift: 0, sizeScale: 1 });
  mat.depthTest = false; mat.depthWrite = false; mat.fog = false;
  const pts = new THREE.Points(geo, mat);
  pts.renderOrder = -20; pts.frustumCulled = false;
  scene.add(pts);
  return mat;
}
...
function addNebulaBackdrops(scene) {
  ...
  const clouds = [
    ...
    { pos: [400, 900, -5200], color: '#7a3060', scale: 7400, op: 0.34 },
  ];
