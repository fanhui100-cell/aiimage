...
💡 Use the **Exam Branch** module in LexiOcean for targeted practice questions.
使用 LexiOcean 的**考试枝路**模块进行专项练习。`
}
...
💡 LexiOcean's **Memory Roots** module handles spaced repetition automatically.
LexiOcean 的**记忆根系**模块自动管理间隔复习。`
}

function generalResponse(level: string): string {
  return `**LexiOcean AI Tutor / AI 导学** 🌊
...
*— LexiOcean AI Tutor (Mock Mode — real AI coming in Phase 3)*`
}
...
*— LexiOcean AI Tutor (Mock Mode — real AI planning in Phase 3)*`
}
