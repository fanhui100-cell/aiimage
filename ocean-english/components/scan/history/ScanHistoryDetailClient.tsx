    ...
    return (
      <div className="theme-light" style={{ minHeight: '100vh', background: 'var(--paper)', paddingTop: '16px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px', textAlign: 'center' }}>
          ...
          <Link href="/scan/history" style={{ color: 'var(--blue-ink)', fontSize: '13px', textDecoration: 'none' }}>
    ...
    <div className="theme-light" style={{ minHeight: '100vh', background: 'var(--paper)', paddingTop: '16px' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          <Link href="/scan/history" style={{ fontSize: '13px', color: 'var(--blue-ink)', textDecoration: 'none' }}>
            ← Back to Scan History / 返回扫描历史
          </Link>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', marginBottom: '12px' }}>
            <div>
              <h1 style={{ margin: '0 0 4px', fontSize: '22px', fontWeight: 700, color: 'var(--ink)', wordBreak: 'break-word' }}>
                {doc.fileName}
              </h1>
          ...
          <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', marginBottom: doc.warnings.length > 0 ? '12px' : 0 }}>
            {[
              ...
              { label: 'Words', value: doc.vocabularyCount, color: 'var(--blue-ink)' },
              { label: 'Notes', value: doc.studyNoteCount, color: 'var(--teal-ink)' },
              { label: 'Warnings', value: doc.warningCount, color: 'var(--gold-ink)' },
          ...
          {doc.warnings.length > 0 && (
            <div style={{
              ...
              {doc.warnings.map((w, i) => (
                <div key={i} style={{ fontSize: '12px', color: 'var(--gold-ink)', marginBottom: i < doc.warnings.length - 1 ? '3px' : 0 }}>
                  ⚠ {w}
                </div>
              ))}
            </div>
        ...
        {(doc.summaryEn || doc.summaryZh) && (
          <div style={{
            ...
            {doc.summaryEn && (
              <p style={{ margin: '0 0 6px', fontSize: '13px', color: 'var(--ink)', lineHeight: 1.7 }}>{doc.summaryEn}</p>
            )}
            {doc.summaryZh && (
              <p style={{ margin: 0, fontSize: '12px', color: 'var(--ink-sub)', lineHeight: 1.7 }}>{doc.summaryZh}</p>
            )}
          </div>
        )}
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            {rawExpanded && (
              <div style={{ marginTop: '8px' }}>
                ...
                <div style={{ marginTop: '6px', fontSize: '11px', color: 'var(--ink-muted)' }}>
        ...
        {doc.studyNotes.length > 0 && (
          <div style={{ marginBottom: '20px' }}>
            <div style={{ fontSize: '11px', letterSpacing: '0.1em', color: 'rgba(14,140,122,0.5)', fontFamily: 'var(--font-mono)', marginBottom: '12px' }}>
                  ...
                  <div key={i} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '10px' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <p style={{ margin: '0 0 4px', fontSize: '12px', color: 'var(--ink)', lineHeight: 1.6 }}>{note.content}</p>
                        <p style={{ margin: 0, fontSize: '12px', color: 'var(--ink-sub)', lineHeight: 1.6 }}>{note.contentZh}</p>
                      </div>
                      <button
          ...
          {!showDeleteConfirm ? (
            ...
            <div style={{
              ...
              <button
                ...
                style={{ padding: '4px 12px', borderRadius: '6px', background: 'rgba(239,68,68,0.2)', border: '1px solid rgba(179,38,30,0.35)', color: '#b3261e', fontSize: '12px', cursor: 'pointer' }}
              >
                Yes / 是
              </button>
              <button
                onClick={() => setShowDeleteConfirm(false)}
                style={{ padding: '4px 10px', borderRadius: '6px', background: 'none', border: '1px solid var(--line)', color: 'var(--ink-sub)', fontSize: '12px', cursor: 'pointer' }}
              >
                Cancel / 取消
              </button>
            </div>
