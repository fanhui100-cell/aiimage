...
from app.services.openai_image import generate_from_reference, generate_from_text
...
def _deduct_credits(db: Session, user_id: str, credits: int) -> None:
...
def _refund_credits(db: Session, user_id: str, credits: int) -> None:
...
async def _read_reference_image(reference_image: UploadFile | None) -> bytes | None:
    if not reference_image:
        return None
    ref_bytes = await reference_image.read()
    if len(ref_bytes) > MAX_UPLOAD_BYTES:
        raise HTTPException(status_code=413, detail="图片文件不能超过 10MB")
    if reference_image.content_type and not reference_image.content_type.startswith("image/"):
...
async def generate_keyword(
    ...
    ref_bytes = await _read_reference_image(reference_image)
    ...
    generation = _save_generation(db, current_user, "keyword", prompt, url, credits, tokens)
...
async def generate_template(
    ...
    generation = _save_generation(db, current_user, "template", prompt, url, credits, tokens, template_id)
    return GenerateResponse(
        id=generation.id,
        image_url=url,
        credits_used=credits,
...
async def generate_custom(
    ...
    generation = _save_generation(db, current_user, "custom", prompt, url, credits, tokens)
    return GenerateResponse(
        id=generation.id,
        image_url=url,
        credits_used=credits,
